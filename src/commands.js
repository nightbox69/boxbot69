...
export function nbDad(client, userName, dadMod, dadChecker, dadCounter) {
  if(dadMod == true) {
      var scrub = userName;
      ...
      if(dadCounter == 1 && dadChecker == false) {
          ...
          //client.say(channel, `/timeout ' ${ someLoserScrub } 60 Nice Try` );
          //client.say(channel, `Wow, ' ${ someLoserScrub } ${ dadCounter } inch?? GitGud boi. LUL`  );
          ...
          //client.say(channel, `/timeout ' + someScrub + ' 20 Thanks for playing Kappa` );
          export var checker = true;
          console.log('Command: ' + checker);
          // setTimeout(function(){ dadChecker = false; client.say(channel, 'nb dad now open OpieOP'); }, 600000);
      } else if(dadChecker == false) {
          //client.say(channel, 'Dad has ' + dadCounter + ' inches of love for you today, '  + scrub);
      } else if(dadChecker == true) {
          //client.say(channel, 'Some scrub by the name of ' + someScrub + ' won the damn game. Check back again later.');
      }
  }
}
